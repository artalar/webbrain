...
async function init() {
  ...
  if (webmcpToggle) webmcpToggle.checked = stored.useWebMcp ?? true;
...
if (webmcpToggle) {
  webmcpToggle.addEventListener('change', async () => {
    await chrome.storage.local.set({ useWebMcp: webmcpToggle.checked }).catch(() => {});
  });
}
